...
- Added `OutpostError::WrongContext` and `OutpostError::MissingOutpostPath` so dispatch-context failures stay in the documented error/exit-code model.
...
- `crates/cli/tests/e2e.rs::dispatch_matrix_contextual_source_and_outpost_commands`
- `crates/cli/tests/e2e.rs::dispatch_matrix_rejects_wrong_contexts`
...
- Initial independent review requested replacing the CLI-only error model and adding matrix-edge tests. Both changes are adopted in the current worktree.
